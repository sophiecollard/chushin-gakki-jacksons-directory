...
fretboardSpecsDecoder =
    map4 FretboardSpecs
        (field "material" string)
        (optionalField "radius" fretboardRadiusDecoder)
        (field "fret_count" int)
        (optionalField "fret_material" string)


fretboardRadiusDecoder : Decoder FretboardRadius
fretboardRadiusDecoder =
    let
        f : String -> Decoder FretboardRadius
        f str =
            case String.toLower str of
                "12_to_16_inches" ->
                    succeed Inches12To16

                _ ->
                    fail "Not a valid fretboard radius"
    in
    string |> andThen f
...
headstockTypeDecoder =
    let
        ...
        f str =
            case String.toLower str of
                "regular" ->
                    succeed Regular6InLine

                "reverse" ->
                    succeed Reverse6InLine

                "regular_3_and_3" ->
                    succeed Regular3And3

                "reverse_3_and_3" ->
                    succeed Reverse3And3
...
priceDecoder =
    oneOf
        [ map2 (\v s -> SimplePrice { value = v, source = s })
            (field "value" string)
            (field "source" linkDecoder)
        , map2 (\vs s -> ComplexPrice { values = vs, source = s })
